...
export class SaltyVoice {
  ...
  private _gameInstanceState: GameInstanceState =
    GameInstanceState.notInitiated;
  ...
  private constructor() {
    ...
    alt.onServer(
      FromServer.updateClientAlive,
      this.onServerUpdateClientAlive.bind(this)
    );
    alt.onServer(
      FromServer.updateClientRange,
      this.onServerUpdateClientRange.bind(this)
    );
    alt.onServer(FromServer.removeClient, this.onServerRemoveClient.bind(this));
    alt.onServer(
      FromServer.phoneEstablish,
      this.onServerPhoneEstablish.bind(this)
    );
    alt.onServer(FromServer.phoneEnd, this.onServerPhoneEnd.bind(this));
    alt.onServer(
      FromServer.radioSetChannel,
      this.onServerRadioSetChannel.bind(this)
    );
    alt.onServer(
      FromServer.radioLeaveChannel,
    ...
    alt.onServer(
      FromServer.playerIsSendingRelayed,
      this.onServerPlayerIsSendingRelayed.bind(this)
    );
    alt.onServer(
      FromServer.updateRadioTowers,
      this.onServerUpdateRadioTowers.bind(this)
    );
    alt.onServer(
      FromServer.isUsingMegaphone,
      this.onServerIsUsingMegaphone.bind(this)
    );
    ...
    loadAnimDict('facials@gen_male@variations@normal').catch((rej) =>
      alt.logError(rej)
    );

    // WebView
    this._webView = new alt.WebView(
      'http://resource/Client/Public/webview.html'
    );
  ...
  private onServerUpdateClient(
    ...
    let voiceClient = new VoiceClient(
      player,
      teamSpeakName,
      voiceRange,
      isAlive,
  ...
  private onServerUpdateClientRange(
    player: alt.Player,
    voiceRange: number
  ): void {
    if (!this.VoiceClients.has(player.id)) return;
  ...
  // Phone
  private onServerPhoneEstablish(
    player: alt.Player,
    position: alt.Vector3
  ): void {
    if (!this.VoiceClients.has(player.id)) return;
    ...
    if (Config.enableSignalStrength) {
      signalDistortion =
        ...
        native.getZoneScumminess(
          native.getZoneAtCoords(position.x, position.y, position.z)
        );
    }
  ...
  private onServerPhoneEnd(playerId: number): void {
    ...
    this.executeCommand(
      new PluginCommand(
        ...
        new PhoneCommunication(voiceClient.teamSpeakName, true)
      )
    );
  }

  // Radio
  private onServerRadioSetChannel(
    radioChannel: string,
    isPrimary: boolean
  ): void {
    if (isPrimary) {
  ...
  private onServerPlayerIsSending(
    ...
    this.onServerPlayerIsSendingRelayed(
      player,
      radioChannel,
      isSending,
      stateChange,
  ...
  private onServerUpdateRadioTowers(radioTowers: RadioTower[]): void {
    ...
    if (this._gameInstanceState != GameInstanceState.connected) return;
    this.executeCommand(
      new PluginCommand(Command.radioTowerUpdate, new RadioTowers(radioTowers))
    );
  }

  // Megaphone
  private onServerIsUsingMegaphone(
    player: alt.Player,
    range: number,
    isSending: boolean,
    position: alt.Vector3
  ...
  private onConnected(): void {
    ...
    alt.emit(
      ToClient.stateChanged,
      this._gameInstanceState,
      this._soundState.microphone,
      this._soundState.speaker
    );
  ...
  private onMessage(messageJson: string): void {
    ...
    switch (message.Command) {
      case Command.pluginState:
        ...
        this.executeCommand(
          new PluginCommand(
            ...
            new RadioTowers(this._configuration.radioTowers)
          )
        );
        break;
      case Command.reset:
      ...
      case Command.talkState:
        this.setPlayerTalking(
          message.Parameter.Name,
          message.Parameter.IsTalking
        );
        break;
    }
  }
  ...
  private playSound(
    ...
    this.executeCommand(
      new PluginCommand(Command.playSound, new Sound(fileName, loop, handle))
    );
  }
  ...
  private stopSound(handle: string): void {
    if (this._gameInstanceState <= 0 || !this._configuration) return;
    this.executeCommand(
      new PluginCommand(Command.stopSound, new Sound(handle))
    );
  }
  ...
  private setPlayerTalking(teamSpeakName: string, isTalking: boolean): void {
    ...
    if (
      ...
      let voiceClient = this.VoiceClients.get(
        this._clientIdMap.get(teamSpeakName)
      );
  ...
  private onClientUseRadio(primaryRadio: boolean, isSending: boolean): void {
    if (primaryRadio) {
      if (isSending != this._radioConfiguration.usingPrimaryRadio) {
        alt.emitServer(
          ToServer.playerIsSending,
          this._radioConfiguration.primaryChannel,
          isSending
        );
      ...
      if (isSending != this._radioConfiguration.usingSecondaryRadio) {
        alt.emitServer(
          ToServer.playerIsSending,
          this._radioConfiguration.secondaryChannel,
          isSending
        );
        if (isSending && Config.enableRadioAnimation)
          native.taskPlayAnim(
